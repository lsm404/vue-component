<template>
  <div class="starry-sky">
    <canvas id="canv" ref="canvas"></canvas>
  </div>
</template>
...
export default {
  props: {
    point: {
      type: Number,
      default: 55 // 生成的星星（点）的个数, 默认25个
    },
    lineColor: {
      type: String,
      default: 'rgba(250,0,0,1)' // 线的颜色
    },
    roundColor: {
      type: String,
      default: 'rgba(0,0,250,1)' // 星星的颜色
    }
  },
  data () {
    return {
  ...
  methods: {
    ...
    drawLine (context, beginX, beginY, closeX, closeY, opacity) {
      context.beginPath()
      context.strokeStyle = `rgba(250, 250, 250, ${opacity})`
